...
BEGIN
  FOR col IN (
    SELECT * FROM (
      SELECT 'delay_reason'      AS cn, 'VARCHAR(1000)' AS dt FROM dual UNION ALL
      SELECT 'comments',              'VARCHAR(2000)'          FROM dual UNION ALL
      SELECT 'tracker_comments',      'VARCHAR(2000)'          FROM dual UNION ALL
      SELECT 'rd_approval_dt',        'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'md50_st',               'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'md50_end',              'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'md50_app_by',           'VARCHAR(200)'           FROM dual UNION ALL
      SELECT 'md50_app_on',           'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'dev_st',                'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'dev_end',               'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'tft_st',                'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'tft_end',               'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'uat_closed_on',         'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'approved1_on',          'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'approved2_on',          'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'approved3_on',          'VARCHAR2(50)'            FROM dual UNION ALL
      SELECT 'deployed_samil',        'VARCHAR(50)'            FROM dual UNION ALL
      SELECT 'deployed_mswil',        'VARCHAR(50)'            FROM dual
    ) cols
  ) LOOP
    DECLARE v NUMBER;
    BEGIN
