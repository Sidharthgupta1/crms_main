...
-- ============================================================
-- STEP 1: Drop old module/approval tables (clean rebuild)
-- ============================================================
DECLARE
  PROCEDURE drop_if(p_tbl VARCHAR2) IS v NUMBER; BEGIN
    SELECT COUNT(*) INTO v FROM user_tables WHERE table_name=UPPER(p_tbl);
    IF v>0 THEN EXECUTE IMMEDIATE 'DROP TABLE '||p_tbl||' CASCADE CONSTRAINTS';
      DBMS_OUTPUT.PUT_LINE('  DROPPED : '||p_tbl); END IF;
  END;
BEGIN
  drop_if('CRMS_RELEASE_APPROVALS');
  drop_if('CRMS_APPROVAL_FLOWS');
  drop_if('CRMS_RELEASE_TASKS');
  drop_if('CRMS_PHASE_TEMPLATES');
  drop_if('CRMS_PHASE_GROUPS');
