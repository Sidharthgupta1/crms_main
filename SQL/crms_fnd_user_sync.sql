...
UPDATE crms_users SET fnd_user_name='SIDHARTH.GUPTA'  WHERE initials='SG';
  ...
  FOR fnd IN (
    SELECT f.user_name,
     ...
     WHERE NVL(f.end_date, SYSDATE+1) > SYSDATE
       AND f.user_name in ('SUP_5783','BNISCHAL','ADAWRA','S10796','S05872','S201')--NOT IN ('GUEST','INITIAL SETUP','SYSADMIN','ANONYMOUS','CONCURRENT MANAGER')
       AND NOT EXISTS (
